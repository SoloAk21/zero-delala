...
if (!BOT_TOKEN || BOT_TOKEN === 'your_telegram_bot_token_here') {
    (async () => {
        ...
        console.log(`[Zero Delala Bot] Shared Regions: ${ETHIOPIAN_REGIONS.length} | Sub-cities: ${ADDIS_ABABA_SUBCITIES.length}`);
        const isBackendReachable = await checkBackendHealth();
        console.log(`[Zero Delala Bot] Backend Health Probe: ${isBackendReachable ? 'CONNECTED (http://localhost:5000)' : 'OFFLINE'}`);
        if (isBackendReachable) {
            ...
            console.log(`[Zero Delala Bot] User Sync Test: ${testSync?.success ? 'SUCCESS (User Created in PostgreSQL)' : 'FAILED'}`);
        }
        console.log('[Zero Delala Bot] Status: READY. Add TELEGRAM_BOT_TOKEN to bot/.env for live Telegram polling.');
        console.log('====================================================');
        process.exit(0);
    })();
}
    ...
    bot.help((ctx) => {
        const helpText = `📖 Zero Delala Guide | መመሪያ:\n\n` +
        ...
        return ctx.reply(helpText, Markup.inlineKeyboard([[Markup.button.webApp('🚀 Launch App Now', WEBAPP_URL)]]));
    });
    bot.command('myproperties', (ctx) => {
        return ctx.reply(`🏢 Manage your real estate listings / የእርስዎን ንብረቶች ያስተዳድሩ:`, Markup.inlineKeyboard([
            [Markup.button.webApp('📋 Manage My Properties', `${WEBAPP_URL}/my-properties`)]
        ]));
    });
    bot.command('saved', (ctx) => {
        return ctx.reply(`❤️ Your bookmarked properties / የተቀመጡ ንብረቶች:`, Markup.inlineKeyboard([
